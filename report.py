...
RESEND_API_KEY = "re_6EGG7JpW_CDu4trSy2HzqNiDXrABsVZLP"
RESEND_FROM = "onboarding@resend.dev"
RESEND_TO = "philippbeer86@gmail.com"  # Updated to use your email

def get_today_messages(supabase_client: SupabaseClient) -> List[Dict]:
    """Fetch all messages from today's date.
    ...
    try:
        ...
        messages = supabase_client.get_tweets_by_date(today)
        return messages
...
def generate_ai_summary(messages_text: str) -> str:
    ...
    try:
        ...
        prompt = f"""You are a crypto analyst creating a daily digest from a list of recent tweets by leading DeFi and protocol-focused Twitter accounts.
...
---
📰 Daily Crypto Protocol Report – {today}
...
def send_email_report(summary: str, statistics: Dict) -> bool:
    ...
    try:
        ...
        # Create email body
        body = f"""
Daily Social Media Summary Report
================================
...
"""
        ...
        response = resend.Emails.send({
            "from": RESEND_FROM,
            "to": RESEND_TO,
            "subject": f"Daily Social Media Summary - {datetime.now().strftime('%Y-%m-%d')}",
            "text": body
        })
...
def main():
    """Main function to generate and display the daily summary."""
    ...
    messages = get_today_messages(supabase)
    if not messages:
        print("No new messages found for today.")
        return
